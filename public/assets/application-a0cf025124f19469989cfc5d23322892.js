...
(function() {
  var bindCancelButton, bindDifficultyButton, bindKeypress, setEditStatus;

  bindCancelButton = function(selector) {
    return $(selector).off('click').on('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      return window.location = $(this).data('dest');
    });
  };

  bindDifficultyButton = function(selector) {
    return $(selector).off('click').on('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
  ...
  bindKeypress = function(selector) {
  ...
  setEditStatus = function(isSaved) {
    if (isSaved) {
  ...
  $(document).on('page:change', function() {
    var quitLightbox, updateLightbox, updateUrl;
    ...
    $('.embed').click(function(e) {
      return window.location = $(this).data('base-url');
    });
    quitLightbox = function() {
      ...
      return window.history.pushState(null, 'GS', newUrl);
    };
    ...
    $('.editable input').on('focus', function() {
      return $(this).parent('.editable').removeClass('saved');
    });
    $('.editable input').on('keyup', function(e) {
      var keycode;
      keycode = e.keyCode;
      if (isPrintableKeycode(keycode)) {
        return $(this).parent('.editable').addClass('edited');
      }
    });
    $('.editable input').on('blur', function() {
      var form;
      form = $(this).parent('.editable');
      if (form.hasClass('edited')) {
        form.removeClass('edited');
        return form.submit();
      }
    });
    $('.editable').on('ajax:success', function(e, data) {
      return $(this).addClass('saved').find('input').blur();
    });
    updateUrl = function(url) {
      return window.location = url;
    };
    updateLightbox = function($element) {
      $('#lightbox').removeClass();
      return $('#lightbox').addClass($element.data('mime'));
    };
    $('#lightbox.text .display').html(marked(decodeURIComponent($('#preview-desc-value').val())));
    return $(document).on('keyup', function(e) {
      var currentId, error, keyEvent, nextId, resourceIndex;
      if (e.keyCode === 39) {
        keyEvent = 'right';
      }
      if (e.keyCode === 37) {
      ...
      if (keyEvent && $('body').hasClass('present') && window.location.search) {
        try {
          resourceIndex = window.location.search.substr(1).split('&').filter(function(k) {
            return k.indexOf('idx') >= 0;
          });
          ...
          if (currentId >= 0) {
            ...
            console.log(nextId);
            if (nextId >= 0) {
              return window.location.search = "p=1&idx=" + nextId;
            }
          }
        } catch (_error) {
          error = _error;
...
(function() {
  $(document).on('page:change', function() {
    return $('.menu-btn').off('click').on('click', function() {
      return $('body').toggleClass('siteMenuOpened');
    });
  });
...
(function() {
  $(document).on('page:change', function() {
    return $('#overlay').off('click').on('click', function() {
      return $('body').toggleClass('siteMenuOpened');
    });
  });
...
(function() {
  var bindCancelButton, bindOnPaste, createMarkdownEditor, initEditorButtons;

  createMarkdownEditor = function(elementId) {
    var codemirror;
    return codemirror = CodeMirror.fromTextArea(document.getElementById(elementId), {
      mode: 'markdown',
  ...
  initEditorButtons = function(editor) {
    var getCursorPosition, getNotSelectedInsertionString, getSelectedInsertionString, insertMarkdown;
    getSelectedInsertionString = function(type) {
      var selection;
      ...
      switch (type) {
        ...
        case 'bold':
          return "** " + selection + " **";
        case 'link':
          return "[" + selection + "](YOUR_URL)";
        case 'image':
          return "![IMAGE_DESCRIPTION](" + selection + ")";
      }
    };
    getNotSelectedInsertionString = function(type) {
      switch (type) {
        case 'h1':
          return "# H1 title here";
        case 'h2':
          return "## H2 title here";
        case 'h3':
          return "### H3 title here";
        case 'italic':
          return "**";
        case 'bold':
          return "*";
        case 'link':
          return "[TEXT_TO_LINK](THE_URL)";
        case 'image':
          return "![IMAGE_DESCRIPTION](IMAGE_URL)";
      }
    };
    getCursorPosition = function(type) {
      return 'end';
    };
    insertMarkdown = function(type) {
      var cursorPosition, insertionNotSelected, insertionSelected, selection, taggedSelection;
      selection = editor.getSelection();
      ...
      taggedSelection = selection ? insertionSelected : insertionNotSelected;
    ...
    $('#h1-btn').on('click', function() {
      return insertMarkdown('h1');
    });
    $('#h2-btn').on('click', function() {
      return insertMarkdown('h2');
    });
    $('#h3-btn').on('click', function() {
      return insertMarkdown('h3');
    });
    $('#italic-btn').on('click', function() {
      return insertMarkdown('italic');
    });
    $('#bold-btn').on('click', function() {
      return insertMarkdown('bold');
    });
    $('#link-btn').on('click', function() {
      return insertMarkdown('link');
    });
    $('#image-btn').on('click', function() {
      return insertMarkdown('image');
    });
    return $('#preview-btn').on('click', function() {
      $('body').toggleClass('preview-mode');
      return $('#markdown-preview').html(marked(editor.getValue()));
    });
  };
  ...
  bindOnPaste = function(selector) {
    return $(selector).off('paste').on('paste', function(e) {
      var input;
      input = this;
      return setTimeout(function() {
        var url;
        url = $(input).val();
        return $.getJSON("http://localhost:3000/preview.json?url=" + url, function(response) {
          $('.preview').html(response.html);
          $('#link-title').val(response.title);
          return $('#link-description').val(response.description);
        });
      }, 100);
    });
  };
  ...
  $(document).on('page:change', function() {
    ...
    if ($('#resource-description').length > 0) {
      editor = createMarkdownEditor('resource-description');
    }
    window.editor = editor;
    initEditorButtons(editor);
    bindCancelButton('.btn-cancel');
    bindOnPaste('#link-url');
    $('#resource_title').on('keypress', function(e) {
      if (e.keyCode === 13) {
        e.preventDefault();
        e.stopPropagation();
        return editor.focus();
    ...
    return $('.description').html(marked(decodeURIComponent($('#desc-value').val())));
  });
...
(function() {
  window.isPrintableKeycode = function(keycode) {
    var valid;
    return valid = (keycode > 47 && keycode < 58) || keycode === 32 || keycode === 13 || keycode === 8 || (keycode > 64 && keycode < 91) || (keycode > 95 && keycode < 112) || (keycode > 185 && keycode < 193) || (keycode > 218 && keycode < 223);
  };
